...
const { sample, cleanMessage, replaceSpoilersWithAsync } = require('./util');
...
async function main() {
  ...
  client.on('message', async msg => {
    ...
    if(
      ...
      await megahal.seedLevelBrainFromString(db, cleanMessage('mythra', msg.cleanContent));
    }
  });
  ...
  client.on('message', async msg => {
    ...
    if(
      ...
      const response = await generateReply(db, cleanMessage('mythra', msg.cleanContent));
      const responseWithFakeSpoilers = await replaceSpoilersWithAsync(response, async () => {
        return await generateReply(db, '');
      });
      await msg.channel.send(responseWithFakeSpoilers);
    }
  });

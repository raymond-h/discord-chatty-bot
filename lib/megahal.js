...
async function seedLevelBrainFromString(db, string) {
  ...
  for(const quads of quadsLists) {
    ...
    for(let i = 0; i < quads.length; i++) {
      ...
      const quadStr = JSON.stringify(quads[i]);

      const ops = [
        { type: 'put', key: `quads\x1F${randomId()}`, value: quadStr },
        { type: 'put', key: 'quads\xFF', value: quadStr },
        { type: 'put', key: `initial-quads\x1F${a}\x1F${randomId()}`, value: quadStr },
        { type: 'put', key: `initial-quads\x1F${b}\x1F${randomId()}`, value: quadStr },
        { type: 'put', key: `initial-quads\x1F${c}\x1F${randomId()}`, value: quadStr },
        { type: 'put', key: `initial-quads\x1F${d}\x1F${randomId()}`, value: quadStr },
